...
User Starts Session — Selects exercise and sets up camera
Real-time Pose Detection — Backend analyzes each frame via /api/pose
Form Validation — App calculates joint angles and compares to expected ranges
Live Feedback — User receives instant corrections ("Keep chest up", "Go deeper")
Rep Tracking — System counts successful reps and records metrics
Session Analysis — After session, comprehensive scoring via /api/score
Progress Tracking — Results stored in Firestore for long-term analysis
